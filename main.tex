		...
		\begin{minipage}{\linewidth}
			\begin{algorithm}[H]
				...
				\strisoluksproc{\strisoluks{$ u $, $ v $, $ \Gamma\sigma $, $ \Omega $}}{
					...
					\nl Compress $ \bigcup_{t \in T} \Theta_t\sigma_t $ into a single coset $ \Theta \theta $\;
					\nl \Return $ \Theta \theta $\;
...
\begin{frame}{Analysis of transitive case of Luks's algorithm}
	...
	\begin{itemize}
		...
		\item $ \Gamma \in \Gamma_d \implies \ImageOf(\psi_\Delta) \in \Gamma_d $ since $ \Gamma_d $ is closed under subgroups
		...
		\] it follows that the transversal size $ \abs{T} \le \abs{D}^{O(1)} $
