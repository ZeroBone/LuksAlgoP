...
\begin{frame}{Lifting non-minimal blocks to block-induced blocks}
	Any blocks $ \Delta \subsetneq \Delta' $ of $ \cdot $ can be lifted to blocks of $ \psi_\Delta $, for example:
	\begin{center}
		\begin{tikzpicture}[scale=1, thick]
			% octagon vertices
		...
		\end{tikzpicture}
	\end{center}
	$ \rightsquigarrow $ Non-minimality of $ \Delta \implies $ Non-primitivity of $ \psi_\Delta $
\end{frame}
...
\begin{frame}{Primitivity and minimality}
	...
	These constructions hold in general are the essence of the proof of:
