...
\begin{frame}{Luks's algorithm}
	\begin{algorithm}[H]
		...
		\KwOut{Coset representation of $ \{\delta \in \Gamma\sigma \mid u(\omega) = v(\omega)\ \forall \omega \in \Omega\} $}
		...
		\luksproc{\luks{$ u $, $ v $, $ \Gamma\sigma $, $ \Omega $}}{
			...
			\nl\If{the action of $ \Gamma $ on $ \Omega $ is not transitive}{
				...
				\nl $ \Gamma'\sigma' := \luks(u, v, \Gamma\sigma, \Omega') $\;
				\nl \Return $ \luks(u, v, \Gamma'\sigma', \Omega \setminus \Omega') $\;
			}
			\nl Compute a minimal block $ \Delta $ of the action of $ \Gamma $ on $ \Omega $ and the associated block system $ \{\gamma \cdot \Delta \mid \gamma \in \Gamma\} $\;
