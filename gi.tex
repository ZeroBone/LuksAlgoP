...
\begin{frame}{Automorphism groups as stabilizers}
	\textbf{Observation}: $ \Aut(G) $ is the stabilizer of $ G $ in the vertex renaming operation of $ \Sym(V(G)) $ on the set of all graphs with vertices $ V(G) $.
...
\begin{frame}{Solutions to $ \GI $ are unions of cosets}
	\begin{itemize}
...
\begin{frame}{Making Luks's theorem even stronger}
	
	Using a similar level-driven approach we can get a better reduction for the bounded-degree case:
