...
\begin{definition}[Graph Isomorphism Problem ($ \GI $)]
	\textbf{Given}: Undirected graphs $ G_0 $, $ G_1 $
...
\begin{frame}{$ \GI $ reduces to $ \SI $ efficiently}
	...
	\begin{proof}
		W.l.o.g. assume that $ G_0 $ and $ G_1 $ have vertex sets $ \{1, \dots, n\} $.
		
		Set $ \Omega := \{\{i, j\} \mid 1 \le i < j \le n\} $ and $ \Sigma := \{0, 1\} $.
		
		For $ b \in \{0, 1\} $ let $ u_b $ be a string defined by $ u_b(\{i, j\}) $.
	\end{proof}
\end{frame}
